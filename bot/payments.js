...
const SUBSCRIPTION_PACKAGES = [
  ...
  },
  {
    key: "like3",
    callback: "buy_like_3",
    menuLabel: "3 ماهه + 1500 لایک  --  539,000 تومان",
...
function registerPaymentHandlers(bot) {
  ...
  bot.on("pre_checkout_query", async (ctx) => {
    try {
      ...
      if (!packageKey) {
        return await ctx.answerPreCheckoutQuery(false, "پکیج نامعتبر است.");
      }
      ...
      if (!findUser) {
        return await ctx.answerPreCheckoutQuery(false, "کاربر یافت نشد.");
      }
